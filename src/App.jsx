  ...
  useEffect(() => {
    ...
    if (resetRequested) {
      timeoutOne = setTimeout(() => {
        setPixels((prePixels) =>
          prePixels.map((pixel) => ({ ...pixel, filled: false }))
        )
      }, 1000)
    }
...
*/
  ...
  const handleMouseEnterPixel = (id) => {
    if (mouseDown) {
  ...
  const pixelElements = pixels.map((pixel) => (
    <div
      ...
      onMouseEnter={() => handleMouseEnterPixel(pixel.id)}
    ></div>
  ))
  ...
  return (
    <div
      ...
      onMouseLeave={handleMouseUp} //mouse wrapper'dan çıkarsa mouseDown false olsun
    >
      <div
